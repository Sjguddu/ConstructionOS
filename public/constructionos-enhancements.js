(() => {
  ...
  style.textContent = `
    ...
    .cos-calc-fab{position:fixed;right:22px;bottom:22px;width:48px;height:48px;border:0;border-radius:15px;background:#0f766e;color:#fff;font-size:20px;font-weight:800;box-shadow:0 12px 28px rgba(15,118,110,.25);cursor:pointer;z-index:9999}
    .cos-calc-fab:hover{background:#0b625c;transform:translateY(-2px)}
    .cos-calc-panel{position:fixed;right:22px;bottom:82px;width:310px;background:#fff;border:1px solid #dfe8e5;border-radius:18px;box-shadow:0 25px 70px rgba(18,52,46,.2);z-index:10000;overflow:hidden}
    ...
    .cos-calc-hint{padding:0 14px 13px;margin:0;color:#8a9692;font-size:8px;text-align:center}
  ...
  function calculator(){
    ...
    const fab=document.createElement('button');fab.className='cos-calc-fab';fab.title='Normal Calculator';fab.setAttribute('aria-label','Open normal calculator');fab.textContent='🧮';
    const panel=document.createElement('div');panel.className='cos-calc-panel';panel.hidden=true;
    panel.innerHTML=`<div class="cos-calc-head"><strong>Normal Calculator</strong><button class="cos-calc-close" aria-label="Close">×</button></div><div class="cos-calc-display">0</div><div class="cos-calc-grid"><button data-k="C" class="op">C</button><button data-k="⌫" class="op">⌫</button><button data-k="%" class="op">%</button><button data-k="/" class="op">÷</button><button data-k="7">7</button><button data-k="8">8</button><button data-k="9">9</button><button data-k="*" class="op">×</button><button data-k="4">4</button><button data-k="5">5</button><button data-k="6">6</button><button data-k="-" class="op">−</button><button data-k="1">1</button><button data-k="2">2</button><button data-k="3">3</button><button data-k="+" class="op">+</button><button data-k="0" class="wide">0</button><button data-k=".">.</button><button data-k="=" class="equal">=</button></div><p class="cos-calc-hint">Keyboard + numpad supported · Enter = result · Esc = close</p>`;
    document.body.append(fab,panel);
    const display=panel.querySelector('.cos-calc-display');let expr='',justCalculated=false;
    const render=()=>display.textContent=expr||'0';
    const press=k=>{
      if(k==='C'){expr='';justCalculated=false;return render()}
      if(k==='⌫'){expr=expr==='Error'?'':expr.slice(0,-1);justCalculated=false;return render()}
      if(k==='='){
        try{if(!expr||!/^[0-9+\-*/%.()\s]+$/.test(expr))throw Error();const result=Function('"use strict";return ('+expr+')')();expr=Number.isFinite(result)?String(Math.round(result*1e10)/1e10):'Error'}catch{expr='Error'}justCalculated=true;return render()
      }
      if(expr==='Error'||(justCalculated&&/^[0-9.]$/.test(k))){expr=k;justCalculated=false;return render()}
      expr+=k;justCalculated=false;render();
    };
    panel.querySelectorAll('[data-k]').forEach(btn=>btn.addEventListener('click',()=>press(btn.dataset.k)));
    const onKey=e=>{if(panel.hidden)return;if(e.key==='Escape'){panel.hidden=true;return}const map={Enter:'=',Backspace:'⌫',Delete:'C','+':'+','-':'-','*':'*','/':'/','%':'%','.':'.'};const k=/^[0-9]$/.test(e.key)?e.key:map[e.key];if(k){e.preventDefault();press(k)}};
    window.addEventListener('keydown',onKey);
    fab.addEventListener('click',()=>{panel.hidden=!panel.hidden;if(!panel.hidden)display.focus?.()});
    panel.querySelector('.cos-calc-close').addEventListener('click',()=>panel.hidden=true);

    // The top-right calculator button now opens this normal calculator,
    // keeping the Civil Engineering calculator separate from the basic tool.
    document.addEventListener('click',e=>{
      const button=e.target.closest?.('.calculator-launch');
      if(!button)return;
